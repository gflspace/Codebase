...
/**
 * Middleware for pre-booking CIS evaluation.
 *
 * Evaluates booking creation requests before processing.
 * Blocks, flags, or allows based on CIS decision.
 */
class CISEvaluateBooking
{
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        try {
            ...
            Log::info('CIS booking evaluation completed', [
                'user_id' => $userId,
    ...
    private function extractProviderId(Request $request): ?string
    {
        return $request->input('provider_id')
            ?? $request->input('service_provider_id')
            ?? null;
    }
    ...
    {
        return [
            'booking_amount' => $request->input('amount') ?? $request->input('total') ?? null,
            'service_type' => $request->input('service_type') ?? $request->input('service') ?? null,
            'scheduled_at' => $request->input('scheduled_at') ?? $request->input('start_time') ?? null,
            'location' => $request->input('location') ?? $request->input('address') ?? null,
